...
2. **TOBE UPDATED** create ./openshift/templates/nsp-fpcare-to-maximus-test.yaml  (copy from -dev.yaml)
   change the IP of proxy to test proxy
   the apply using:
```console
oc process -f nsp-fpcare-to-maximus-test.yaml \
...
4. allow the test project to pull from tools:
   Go to the test project (oc project 3f9283-test).
```console
...
c) Apply the quickstart, apps can access all (for test, make sure your default oc project is test):

```console
oc process -f openshift/templates/quickstart.yaml \
    NAMESPACE_PREFIX=3f9283 -p ENVIRONMENT=test | \
    oc apply -f -
```
